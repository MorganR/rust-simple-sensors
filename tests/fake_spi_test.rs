...
fn read_multiple() -> nb::Result<(), SpiError> {
    ...
    spi.try_send(0)?;
    assert_eq!(spi.try_read()?, 1);

    spi.try_send(0)?;
    assert_eq!(spi.try_read()?, 2);
    Ok(())
}
...
fn write_asynchronous() -> nb::Result<(), SpiError> {
    ...
    let async_result = spi.try_send(4);
